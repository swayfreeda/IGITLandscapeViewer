	...
	, m_LODPendingRefresh(false)
{
	...
	setWindowTitle(QString("3D 视角 %1").arg(m_uniqueID));
	...
	//auto-load previous perspective settings
	{
		...
		if (m_customLightEnabled)
		    ...
		    displayNewMessage("警告: 普通光照开启",ccGLWindow::LOWER_LEFT_MESSAGE,false,2,CUSTOM_LIGHT_STATE_MESSAGE);
		if (!m_sunLightEnabled)
			...
			displayNewMessage("警告: 太阳光照关闭",ccGLWindow::LOWER_LEFT_MESSAGE,false,2,SUN_LIGHT_STATE_MESSAGE);
	}
}
...
void ccGLWindow::initializeGL()
{
	...
	if (!m_silentInitialization)
	{
		...
		ccLog::Print("[3D 视角 %i] GL 版本: %s",m_uniqueID,glGetString(GL_VERSION));
		ccLog::Print("[3D 视角 %i] 显卡制造商: %s",m_uniqueID,vendorName);
	}
	...
	if (ccFBOUtils::CheckVBOAvailability())
	{
		...
		else if (!m_silentInitialization)
		{
			...
			ccLog::Print("[3D 视角 %i] VBOs 有效",m_uniqueID);
		}
	}
	else
	{
	...
	if (!m_shadersEnabled)
	{
		...
		if (!m_silentInitialization)
			...
			ccLog::Warning("[3D 视角 %i] 着色器 和 GL 过滤器无效",m_uniqueID);
	}
	else
	{
		if (!m_silentInitialization)
			//ccLog::Print("[3D View %i] Shaders available",m_uniqueID);
			ccLog::Print("[3D 视角 %i] 着色器有效",m_uniqueID);
		...
		if (m_glFiltersEnabled)
		{
			if (!m_silentInitialization)
				...
				ccLog::Print("[3D 视角 %i] GL 过滤器有效",m_uniqueID);
			m_alwaysUseFBO = true;
		}
		else if (!m_silentInitialization)
		{
			//ccLog::Warning("[3D 视角 %i] GL filters unavailable (FBO not supported)",m_uniqueID);
			ccLog::Warning("[3D 视角 %i] GL 过滤器 无效 (FBO 不被支持)",m_uniqueID);
		}
		...
		if (!m_colorRampShader)
		{
			...
			else
			{
				...
				{
					if (!m_silentInitialization)
						ccLog::Warning(QString("[3D 视角 %1] 加载着色器失败: '%2'").arg(m_uniqueID).arg(error));
				...
				{
					if (!m_silentInitialization)
						//ccLog::Print("[3D 视角 %i] Color ramp shader loaded successfully",m_uniqueID);
						ccLog::Print("[3D 视角 %i] 着色器加载成功",m_uniqueID);
	...
	if (!m_silentInitialization)
		...
		ccLog::Print("[ccGLWindow] 3D 视角初始化完成！");
}
...
void ccGLWindow::resizeGL(int w, int h)
{
	...
	displayNewMessage(QString("窗口大小 = %1 * %2 (px)").arg(w).arg(h),
...
void ccGLWindow::setPerspectiveState(bool state, bool objectCenteredView)
{
	...
	if (m_viewportParams.perspectiveView)
	{
		...
		//display message
		displayNewMessage(objectCenteredView ?	/*"Centered perspective ON"*/"物体中心透视：开启" : /*"Viewer-based perspective ON"*/" 观察者透视：ON",
												ccGLWindow::LOWER_LEFT_MESSAGE,
												false,
												2,
...
bool ccGLWindow::InitGLEW()
{
	...
	if (!ccFBOUtils::InitGLEW())
	{
		...
		ccLog::Warning("[Glew] 初始化OpengGL拓展库发生错误!");
		return false;
	}
	else
	{
		//ccLog::Print("[Glew] Initialized!");
		ccLog::Print("[Glew] 初始化完成!");
		return true;
	}
