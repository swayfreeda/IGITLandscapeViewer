...
CC_FILE_ERROR ObjFilter::loadFile(QString filename, ccHObject& container, LoadParameters& parameters)
{
	...
	ccLog::Print(QString("[OBJ] 文件") + filename);
	...
	pDlg.setMethodTitle("OBJ 文件");
	//pDlg.setInfo("Loading in progress...");
	pDlg.setInfo("正在加载中，请稍后...");
	pDlg.setRange(0,0);
	pDlg.show();
	...
	unsigned lineCount = 0;//行数
	unsigned polyCount = 0;//折线个数
	QString currentLine = stream.readLine(); //读取行
	while (!currentLine.isNull()){
		...
		//skip comments & empty lines //注释行和空白行 跳过
		...
		/*** new vertex ***/ //读取顶点
		if (tokens.front() == "v")	{
			//reserve more memory if necessary //判断内存
			...
			if (pointsRead == 0){
				if (HandleGlobalShift(Pd,Pshift,parameters)){
					...
					ccLog::Warning("[OBJ] 点云重新计算中心! 平移: (%.2f,%.2f,%.2f)",Pshift.x,Pshift.y,Pshift.z);
				}
			}

			//shifted point //讲点进行平移
		...
		/*** new vertex texture coordinates ***/  //读取纹理坐标
		...
		else if (tokens.front().startsWith('f'))
		{
			...
			for (int i=1; i<tokens.size(); ++i)	{
				...
				{
					fe.vIndex = vertexTokens[0].toInt(); //定点的索引
					if (vertexTokens.size()>1 && !vertexTokens[1].isEmpty())
						fe.tcIndex = vertexTokens[1].toInt();//纹理坐标的索引
					if (vertexTokens.size()>2 && !vertexTokens[2].isEmpty())
						fe.nIndex = vertexTokens[2].toInt(); //法向量的索引
				}
			...
			if (currentFace.size() < 3){
				...
				ccLog::Error("面片错误: 位于 %1 行的面片顶点个数小于3!",lineCount);
				error = true;
				break;
			}
		...
		/*** material file (MTL) ***/
			...
			if (tokens.size() < 2 || tokens[1].isEmpty()){
				...
				ccLog::Print(QString("[OBJ] 材料文件: ")+mtlFilename);
				...
				if (ccMaterialSet::ParseMTL(mtlPath,mtlFilename,*materials,errors)){
					ccLog::Print("[OBJ] %i 材料加载完毕",materials->size()-oldSize);
					...
					ccLog::Error(QString("[OBJ] 加载材料文件失败! (应当位于 '%1')").arg(mtlPath+'/'+QString(mtlFilename)));
	...
	if (!error){
		...
		ccLog::Print("[OBJ] %i 定点, %u 面片",pointsRead,totalFacesRead);
		if (texCoordsRead > 0 || normsRead > 0)
			//ccLog::Print("[OBJ] %i tex. coords, %i normals",texCoordsRead,normsRead);
			ccLog::Print("[OBJ] %i 纹理坐标, %i 法向量",texCoordsRead,normsRead);
		...
		if (!error && baseMesh)	{
			...
			if (!normals){
				...
				//else
				{
				   ...
				   ccLog::Warning("[OBJ] 网格不存在法向量! 您需要重新计算法向量 (执行 \"编辑 > 法向量 > 计算\")");
				}
			}

			//create sub-meshes if necessary
			//ccLog::Print("[OBJ] 1 mesh loaded - %i group(s)", groups.size());
			ccLog::Print("[OBJ] 1 网格加载完成 - %i 组(s)", groups.size());
			if (groups.size() > 1){
