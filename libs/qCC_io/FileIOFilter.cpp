...
void FileIOFilter::InitInternalFilters()
{
	...
	Register(Shared(new PMVSCamFilter()));
										...
										Shared filter)
{
	...
	if (!fi.exists()){
		...
		ccLog::Error(QString("[Load] 文件 '%1' 不存在!").arg(filename));
		return 0;
	}
	...
	try	{
		...
		ccLog::Warning(QString("[I/O] 加载文件 '%1' 时出现内部异常").arg(filename));
		if (container)
	...
	if (result != CC_FERR_NO_ERROR)
		...
		ccLog::Print(QString("[I/O] 文件 '%1' 加载成功").arg(filename));
										...
										QString fileFilter/*=QString()*/)
{
	...
	if (!fileFilter.isEmpty()){
		...
		if (!filter)	{
			...
			ccLog::Error(QString("[Load] 内部错误: 没有文件过滤器对应 '%1'").arg(fileFilter));
			return 0;
		}
	}
	else //we need to guess the I/O filter based on the file format
	{
		...
		if (extension.isEmpty()){
			...
			ccLog::Error("[Load] 无法获取文件格式: 没有文件扩展名");
			return 0;
		}
		...
		if (!filter)
		{
			...
			ccLog::Error(QString("[Load] 无法获取文件格式: 不能识别的文件扩展名 '%1'").arg(extension));
			return 0;
		}
	}
