...
//! Sensor types 
enum CC_SENSOR_TYPE {	UNKNOWN_SENSOR,  //未知传感器
						GROUND_BASED_LIDAR,  //地面传感器
};
...
public:
	...
	//inherited from ccHObject //从 ccHObject 中继承的
	...
	**/
	virtual CC_SENSOR_TYPE getType() const { return UNKNOWN_SENSOR; } //需要重新实现
	...
	**/
	...
	//! Returns associated positions  //返回位置信息 数据结构式buffer
	ccIndexedTransformationBuffer* getPositions()  { return m_posBuffer; }

	//! Sets associated positions   //设置位置信息 数据结构式buffer
...
protected:
	...
	virtual bool toFile_MeOnly(QFile& out) const; // 存储文件
	virtual bool fromFile_MeOnly(QFile& in, short dataVersion, int flags); // 读取文件

	//inherited from ccHObject
	virtual void applyGLTransformation(const ccGLMatrix& trans); // 从ccHObject中继承

	//! Positions buffer (optional) // 位置缓冲器 // 存储一系列相机位置
	...
	**/
	...
	double m_activeIndex;  // 当前的索引

	//! Color of the sensor
	/** Default color is green. 
	**/
	ccColor::Rgb m_color;  // 默认的颜色

	//! Sensor graphic representation scale
	PointCoordinateType m_scale;  // 图表示尺度???????
};
