...
public:
	...
	**/
	...
	bool triherdonsShown() { return m_showTrihedrons; }//返回显示三面体

	//! [Display option] Sets whether trihedrons should be displayed or not (otherwise only points or a polyline)
	void showTriherdons(bool state) { m_showTrihedrons = state; } //设置显示三面体

	//! [Display option] Returns trihedron display size //返回三面体显示尺度
	float triherdonsDisplayScale() { return m_trihedronsScale; }

	//! [Display option] Sets trihedron display size //设置三面体显示尺度
	...
	void showPathAsPolyline(bool state) { m_showAsPolyline = state; } //将路径表示折线
