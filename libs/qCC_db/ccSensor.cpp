...
ccColor::Generator colorGen;
ccSensor::ccSensor(QString name)
	: ccHObject(name)           //名称
	, m_posBuffer(0)            // 缓冲器
	, m_activeIndex(0)          //激活索引
	, m_color(colorGen.Random(true))//m_color(ccColor::green)  // 默认颜色
	, m_scale(PC_ONE){
	m_rigidTransformation.toIdentity(); //刚体变换初始化为单位矩阵
}

//=============================================================================//
ccSensor::ccSensor(const ccSensor &sensor) //复制构造函数
	: ccHObject(sensor)
	, m_posBuffer(0)
	, m_rigidTransformation(sensor.m_rigidTransformation) //刚体变换
	, m_activeIndex(sensor.m_activeIndex)//ACTIVE索引
	, m_color(sensor.m_color) // 颜色
	, m_scale(sensor.m_scale) // 尺度
{
	if (sensor.m_posBuffer) //位置
		m_posBuffer = new ccIndexedTransformationBuffer(*sensor.m_posBuffer);
}
//==============================================================================//
bool ccSensor::addPosition(ccGLMatrix& trans, double index) //添加位置信息
{
	if (!m_posBuffer){
