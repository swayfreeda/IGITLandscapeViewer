...
ccDBRoot::ccDBRoot(ccCustomQTreeView* dbTreeWidget, QTreeView* propertiesTreeWidget, QObject* parent) : QAbstractItemModel(parent)
{
	...
	m_dbTreeWidget->setModel(this); //设置Model model一般为自定义
	m_dbTreeWidget->header()->hide();
	
	//drag & drop support // 拖放操作
	...
	m_dbTreeWidget->setDragDropMode(QAbstractItemView::InternalMove);//进行移动
#ifndef CC_QT5
	...
	m_dbTreeWidget->setContextMenuPolicy(Qt::CustomContextMenu); //出现右键菜单
	...
	m_sortSiblingsType = new QAction("按类型排序",this);

	//m_sortSiblingsAZ = new QAction("Sort siblings by name (A-Z)",this);
	m_sortSiblingsAZ = new QAction("按名称排序 (A-Z)",this);

	//m_sortSiblingsZA = new QAction("Sort siblings by name (Z-A)",this);
	m_sortSiblingsZA = new QAction("按名称排序 (Z-A)",this);
	...
	m_toggleSelectedEntitiesVisibility = new QAction("切换可视性",this);
	...
	m_toggleSelectedEntitiesNormals = new QAction("切换法向量",this);
	
	//m_toggleSelectedEntitiesMat = new QAction("Toggle materials/textures",this);
	m_toggleSelectedEntitiesMat = new QAction("切换材料/纹理",this);
	
	//m_toggleSelectedEntitiesSF = new QAction("Toggle SF",this);
	m_toggleSelectedEntitiesSF = new QAction("切换标量场(SF)",this);
	...
	m_alignCameraWithEntity = new QAction("对齐相机",this);
	
	//m_alignCameraWithEntityReverse = new QAction("Align camera (reverse)",this);
	m_alignCameraWithEntityReverse = new QAction("对齐相机 (反向)",this);
	...
	m_propertiesModel = new QStandardItemModel(0, 2, parent); // QStandardItemModel是从QAbstractItemModel继承过来的，Qt定义好的 如果有特殊需求需要自己从QAbstractItemModel中继承
	/*//already done in ui file!
	...
	m_propertiesTreeWidget->setModel(m_propertiesModel); //设置模型
#ifdef CC_QT5
...
void ccDBRoot::unloadAll()
{
	...
	while (m_treeRoot->getChildrenNumber() > 0){
		...
		anObject->prepareDisplayForRefresh_recursive();//将anObject与其子物体全部重新显示一遍

		beginRemoveRows(index(anObject).parent(),i,i); //删除行前必须调用 beginRemoveRows() 和 removeChild(i)
...
void ccDBRoot::addElement(ccHObject* anObject, bool autoExpand/*=true*/)
{
	...
	beginInsertRows(insertNodeIndex, childPos, childPos); // parent的位置，插入行的起止
	...
	if (autoExpand)	{
		...
		if (childIndex.isValid())
			m_dbTreeWidget->expand(childIndex); //将孩子节点展开，注意所有的额操作都是在ModelIndex上进行的
	}
	else{   //if (parentObject)
		m_dbTreeWidget->expand(insertNodeIndex);//将父亲物体展开
	}
}
//================================================expandElement=================================================//
void ccDBRoot::expandElement(ccHObject* anObject, bool state)
{
	...
	m_dbTreeWidget->setExpanded(index(anObject),state); //将Object展开
}
//=========================================================removeElment=====================================================//
void ccDBRoot::removeElement(ccHObject* anObject)
{
	...
	//we restore properties view//更新属性列表
	updatePropertiesView();
}
	...
	if (index.isValid()){	
		...
		if (role == Qt::EditRole){
			...
			if (item){
				...
				// - name might be displayed in 3D //三维场景重新绘制
...
//===================================================index================================================================//
//获取位于parent第row个子物体的modelIndex
QModelIndex ccDBRoot::index(int row, int column, const QModelIndex &parentIndex) const
{
	if (!hasIndex(row, column, parentIndex)) //判断是否尊在该ModelIndex
		return QModelIndex();
...
QModelIndex ccDBRoot::index(ccHObject* object)
{
	...
	if (object == m_treeRoot)return QModelIndex(); //物体时树的根节点
	...
	if (!parent)	{
		...
		ccLog::Error(QString("创建DB树索引时发生错误: 物体 '%1' 没有父亲物体!").arg(object->getName()));
		return QModelIndex();
	}

	int pos = parent->getChildIndex(object);
	assert(pos >= 0); //在父亲节点的位置
...
void ccDBRoot::unselectEntity(ccHObject* obj)
{
	if (obj && obj->isSelected()){
		...
		QModelIndex objIndex = index(obj); //obj位于父亲物体的索引
		if (objIndex.isValid())	{
			QItemSelectionModel* selectionModel = m_dbTreeWidget->selectionModel();
			assert(selectionModel);
			//将objIndex从selectionModel中删除
...
{
	//将Obj交给托管
	m_ccPropDelegate->fillModel(obj); //创建TreeView中的基本视图，但是Matrix和RGB相关的对画框是怎么设置的？？？
...
{
	assert(m_propertiesTreeWidget);
	QItemSelectionModel* qism = m_dbTreeWidget->selectionModel(); //选择模型
	QModelIndexList selectedIndexes = qism->selectedIndexes(); //选中的物体的索引
	if (selectedIndexes.size() == 1)
...
void ccDBRoot::gatherRecursiveInformation()
{
	...
	//output information
	{
		...
		infoStr << QString("点(s):\t\t%1").arg(locale.toString(info.pointCount));
		infoStr << QString("三角面片(s):\t\t%1").arg(locale.toString(info.triangleCount));

		infoStr << separator;
		if (info.colorCount)
			//infoStr << QString("Color(s):\t\t%1").arg(locale.toString(info.colorCount));
			infoStr << QString("颜色(s):\t\t%1").arg(locale.toString(info.colorCount));
		if (info.normalCount)
			//infoStr << QString("Normal(s):\t\t%1").arg(locale.toString(info.normalCount));
			infoStr << QString("法向量(s):\t\t%1").arg(locale.toString(info.normalCount));

		if (info.scalarFieldCount)
			//infoStr << QString("Scalar field(s):\t\t%1").arg(locale.toString(info.scalarFieldCount));
			infoStr << QString("标量场(s):\t\t%1").arg(locale.toString(info.scalarFieldCount));
		if (info.materialCount)
			//infoStr << QString("Material(s):\t\t%1").arg(locale.toString(info.materialCount));
			infoStr << QString("材料(s):\t\t%1").arg(locale.toString(info.materialCount));

		infoStr << separator;
		//infoStr << QString("Cloud(s):\t\t%1").arg(locale.toString(info.cloudCount));
		infoStr << QString("点云(s):\t\t%1").arg(locale.toString(info.cloudCount));
		infoStr << QString("网格(s):\t\t%1").arg(locale.toString(info.meshCount));
		...
		if (info.octreeCount)
		    ...
		    infoStr << QString("八叉树(s):\t\t%1").arg(locale.toString(info.octreeCount));
		if (info.imageCount)
			//infoStr << QString("Image(s):\t\t%1").arg(locale.toString(info.imageCount));
			infoStr << QString("图像(s):\t\t%1").arg(locale.toString(info.imageCount));
		if (info.labelCount)
			//infoStr << QString("Label(s):\t\t%1").arg(locale.toString(info.labelCount));
			infoStr << QString("标签(s):\t\t%1").arg(locale.toString(info.labelCount));
		if (info.sensorCount)
			//infoStr << QString("Sensor(s):\t\t%1").arg(locale.toString(info.sensorCount));
			infoStr << QString("传感器(s):\t\t%1").arg(locale.toString(info.sensorCount));
...
void ccDBRoot::addEmptyGroup()
{
	...
	//获取conextMenuPos位置的 模型索引 Model Index
